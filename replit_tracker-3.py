...
def get_analytics_sheet():
    ...
    if _analytics_sheet: return _analytics_sheet
    try:
        ...
        try:
            _analytics_sheet = wb.worksheet(ANALYTICS_TAB)
        except gspread.exceptions.WorksheetNotFound:
            _analytics_sheet = wb.add_worksheet(ANALYTICS_TAB, rows=5000, cols=30)
            _analytics_sheet.append_row([
                'pair','direction','session','zone_src',
                'entry','sl','tp1','tp2','rr_tp1','rr_tp2','sl_pips',
                'sweep_time','fvg_formed_at','fired_at',
                'entry_triggered_at','tp1_hit_at','final_outcome_at',
                'outcome','final_price','mae','mfe',
                'sweep_to_fvg_min','fvg_to_entry_min','fired_to_entry_min',
                'entry_to_tp1_min','tp1_to_final_min','total_trade_min'
            ])
            print(f"[ANALYTICS] Created '{ANALYTICS_TAB}' tab with headers")
        print("[ANALYTICS] Sheet connected ✓")
        return _analytics_sheet
    except Exception as e:
        print(f"[ANALYTICS] Sheet error: {e}"); return None

def log_analytics(sig, outcome, final_price):
    """Append completed trade analytics to Google Sheets — persists across redeploys."""
    try:
        now        = datetime.now(timezone.utc)
        entry_time = sig.get('entry_time', '')
        tp1_time   = sig.get('tp1_hit_at', '')
        fvg_formed = sig.get('fvg_formed_at', '')

        def parse_ts(s):
            if not s: return None
            try: return datetime.fromisoformat(s.replace(' UTC', '+00:00'))
            except Exception: return None

        entry_dt = parse_ts(entry_time)
        tp1_dt   = parse_ts(tp1_time)
        fvg_dt   = parse_ts(fvg_formed)
        fired_dt = parse_ts(sig.get('fired_at', ''))

        def mins(a, b):
            if a and b: return round((b - a).total_seconds() / 60, 1)
            return ''

        row = [
            sig.get('pair',''),          sig.get('direction',''),
            sig.get('session',''),       sig.get('zone_src',''),
            sig.get('entry',''),         sig.get('sl',''),
            sig.get('tp1',''),           sig.get('tp2',''),
            sig.get('rr_tp1',''),        sig.get('rr_tp2',''),
            sig.get('sl_pips',''),       sig.get('sweep_time',''),
            fvg_formed,                  sig.get('fired_at',''),
            entry_time,                  tp1_time,
            now.strftime('%Y-%m-%d %H:%M:%S UTC'),
            outcome,                     final_price,
            sig.get('mae',''),           sig.get('mfe',''),
            mins(parse_ts(sig.get('sweep_time','')), fvg_dt),
            mins(fvg_dt, entry_dt),      mins(fired_dt, entry_dt),
            mins(entry_dt, tp1_dt),
            mins(tp1_dt, now) if tp1_time else '',
            mins(entry_dt, now),
        ]
        ...
        if sheet:
            ...
            print(f"[ANALYTICS] {sig.get('pair')} {outcome} → Sheets ✓ "
                  f"total={mins(entry_dt,now)}m mae={sig.get('mae')} mfe={sig.get('mfe')}")
    except Exception as e:
...
def _poll():
    ...
    for i, key in enumerate(keys):
        ...
        # ── Update MAE / MFE ─────────────────────────────────────────────
        # MAE: worst price against position since entry
        # MFE: best price in favor since entry
        with trades_lock:
            if direction == 'short':
                mae = max(sig.get('mae') or price, price)  # highest price = worst for short
                mfe = min(sig.get('mfe') or price, price)  # lowest price = best for short
            else:
                mae = min(sig.get('mae') or price, price)  # lowest price = worst for long
                mfe = max(sig.get('mfe') or price, price)  # highest price = best for long
            active_trades[key]['mae'] = mae
            active_trades[key]['mfe'] = mfe
        ...
        if not tp1_hit:
            ...
            if tp1_reached:
                ...
                stats["tp1_hits"] += 1
                with trades_lock:
                    active_trades[key]['tp1_hit']    = True
                    active_trades[key]['tp1_hit_at'] = now
                alert_tp1(sig); sheet_tp1(sig)
            elif sl_reached:
                print(f"[MONITOR] {pair} LOSS @ {price:.{dp}f}")
                stats["losses"] += 1
                with trades_lock: sig = active_trades[key]
                alert_final(sig, 'LOSS', price); sheet_final(sig, 'LOSS')
                log_analytics(sig, 'LOSS', price)
                resolved.append(key)
        else:
            ...
            if tp2_reached:
                ...
                log_analytics(sig, 'FULL_WIN', price)
                resolved.append(key)
            elif be_reached:
                ...
                log_analytics(sig, 'TP1_BE', price)
                resolved.append(key)
        ...
        for row in sheet.get_all_values()[1:]:
            ...
            try:
                ...
                sig = {
                    ...
                    'rr_tp1':        float(row[7]) if row[7] else 1.5,
                    'rr_tp2':        float(row[8]) if row[8] else 3.0,
                    'sl_pips':       float(row[9]) if len(row) > 9 and row[9] else None,
                    ...
                    'entry_time':    row[14].strip() if len(row) > 14 else '',
                    'fvg_formed_at': '',   # not in sheet — populated by signal at creation
                    'tp1_hit':       tp1_hit,
                    'tp1_hit_at':    row[15].strip() if tp1_hit and len(row) > 15 else '',
                    'status':        'ACTIVE',
                    'mae':           None,
                    'mfe':           None,
                }
