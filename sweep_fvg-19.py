"""
EDGE Sweep+FVG Strategy Module — v14
=====================================
...
Strategy Logic:
  1. H4 EQ zones identified (BSL/SSL) — 120 candle lookback
     Layer 2: mitigation filter (close must exceed boundary
              by 0.20 × ATR to count as genuine breakout)
     Layer 3: recency proximity (approached within 20 H4 bars)

  2. M15 price closes inside zone → N=14 counter starts
     Zone must hold (no close beyond boundary + 0.20 × ATR buffer)
...
Changes vs v13:
  · ZoneManager.MITIGATION_BUFFER = 0.20 × ATR added
    Layer 2 mitigation: close must exceed zone boundary by
    0.20 × H4 ATR to be counted as a genuine breakout.
    Previously any close 1 pip beyond boundary killed the zone.
  · SweepDetector._zone_holds() uses same 0.20 × ATR buffer
    so N=14 monitoring window is consistent with Layer 2 logic.
  · No other logic changed — FVGScanner, RiskManager, DedupManager,
    SignalBuilder all unchanged.
"""
...
class ZoneManager:
    ...
    MITIGATION_BUFFER = 0.20  # ATR fraction a close must exceed
                               # zone boundary by to count as a
                               # genuine breakout. Prevents 1-3 pip
                               # noise closes from killing live zones.
    ...
    def _apply_mitigation(self, zones: list) -> list:
        """
        Remove zones where price closed MEANINGFULLY through boundary
        AFTER the zone was formed.

        BSL dead: subsequent H4 close > zone_top + MITIGATION_BUFFER × ATR
        SSL dead: subsequent H4 close < zone_bottom - MITIGATION_BUFFER × ATR

        The buffer (0.20 × ATR) prevents 1–3 pip noise closes from
        killing structurally intact zones. A genuine breakout will close
        well beyond the boundary; a wick or spread spike will not clear
        the buffer.
        """
        buf  = self.MITIGATION_BUFFER * self.atr
        live = []
        ...
        for zone in zones:
            ...
            for c in subsequent:
                if zone['side'] == 'BSL' and c['close'] > zone['top'] + buf:
                    mitigated = True
                    break
                if zone['side'] == 'SSL' and c['close'] < zone['bottom'] - buf:
                    mitigated = True
                    break
...
class SweepDetector:
    ...
    def _zone_holds(self, zone: dict,
                    entry_i: int, sweep_i: int) -> bool:
        """
        Within N=14 window, confirm no M15 candle closed
        MEANINGFULLY beyond zone boundary before the sweep.

        Same MITIGATION_BUFFER logic as ZoneManager — uses 0.20 × ATR
        so that a single spike close 1–2 pips beyond the boundary
        doesn't cancel a pending sweep setup.

        BSL: no close above zone_top + 0.20 × ATR
        SSL: no close below zone_bottom - 0.20 × ATR
        """
        buf = 0.20 * self.atr
        for i in range(entry_i, sweep_i):
            c = self.candles[i]
            if zone['side'] == 'BSL' and c['close'] > zone['top'] + buf:
                return False
            if zone['side'] == 'SSL' and c['close'] < zone['bottom'] - buf:
    ...
    def _find_sweep(self, zone: dict, entry_i: int) -> dict | None:
        """
        Within N=14 candles from zone entry:
          - Monitor zone integrity
          - Search for valid sweep candle
          - N exhausted → ACCUMULATION, discard
        Returns sweep dict including n_counter_value
        (which N bar the sweep fired at — analytics data).
        """
        ...
        for i in range(entry_i, window_end):
            ...
            if not self._zone_holds(zone, entry_i, i):
                logger.info(
                    f"[{self.pair}] Zone {zone['side']} "
                    f"{zone['top']:.5f} broken — BREAKOUT"
                )
                return None

            if self._is_valid_sweep(c, zone):
                n_counter_value = i - entry_i + 1
                logger.info(
                    f"[{self.pair}] ✅ Sweep | "
                    f"{zone['side']} @ {c['time']} "
                ...
                return {
                    ...
                    'entry_i'         : entry_i,
                    'atr'             : self.atr,
                    'n_counter_value' : n_counter_value,  # analytics
                }

        logger.info(
            f"[{self.pair}] N=14 exhausted — "
            f"{zone['side']} {zone['top']:.5f} likely accumulation"
        )
        return None
    ...
    def detect(self) -> list:
        ...
        for side in ('bsl', 'ssl'):
            for zone in self.zones[side]:
                entry_i = self._find_zone_entry(zone, start_i=0)
                if entry_i is None:
                    continue
                sweep = self._find_sweep(zone, entry_i)
                if sweep is None:
                    continue
                sweeps.append(sweep)
...
class DedupManager:
    ...
    def _prune(self):
        """Remove expired time-based keys. Zone keys kept until released."""
        ...
        for key, meta in self._fired.items():
            ...
            try:
                ...
                ttl      = (
                    self.SWEEP_TTL_MINS if key.startswith('SWEEP_')
                    else self.SIGNAL_TTL_MINS
                )
                if age_mins > ttl:
                    drop.append(key)
            except Exception:
    ...
    def is_zone_fired(self, zone: dict) -> bool:
        return self._zone_key(zone) in self._fired
...
class SignalBuilder:
    ...
    def _build_message(self, sig: dict) -> str:
        ...
        return (
            ...
            f"_FVG confirmed. MSS validated. Set pending order._\n\n"
            f"Direction : `{side}`\n"
