...
ANALYTICS_FILE = 'trade_analytics.json'
...
def log_analytics(sig, outcome, final_price):
    """Append completed trade analytics record — no extra API calls."""
    try:
        ...
        record = {
            ...
            'zone_src':           sig.get('zone_src'),
            'entry':              sig.get('entry'),
            'sl':                 sig.get('sl'),
            'tp1':                sig.get('tp1'),
            'tp2':                sig.get('tp2'),
            'rr_tp1':             sig.get('rr_tp1'),
            'rr_tp2':             sig.get('rr_tp2'),
            ...
            'tp1_hit_at':         tp1_time,
            'final_outcome_at':   now.strftime('%Y-%m-%d %H:%M UTC'),
            'outcome':            outcome,
            'final_price':        final_price,
            'mae':                sig.get('mae'),
            'mfe':                sig.get('mfe'),
            # Durations in minutes
            'sweep_to_fvg_min':   mins(parse_ts(sig.get('sweep_time')), fvg_dt),
            'fvg_to_entry_min':   mins(fvg_dt, entry_dt),
            'fired_to_entry_min': mins(fired_dt, entry_dt),
            'entry_to_tp1_min':   mins(entry_dt, tp1_dt),
            'tp1_to_final_min':   mins(tp1_dt, now) if tp1_time else None,
            'total_trade_min':    mins(entry_dt, now),
        }
        data = load_analytics()
        data.append(record)
        save_analytics(data)
        print(f"[ANALYTICS] {sig.get('pair')} {outcome} — "
              f"total={record['total_trade_min']}m "
              f"mae={record['mae']} mfe={record['mfe']}")
