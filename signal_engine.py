...
def main():
    ...
    def _ensure(pair, cache):
        if pair not in cache:
            cache[pair] = {"h4": {"candles": [], "fetched_hour": -1}}

    if h4_needed:
        print(f"\n[1b] HTF refresh — H4=True")
    ...
    if SWEEP_FVG_ENABLED:
        ...
        for pair in SWEEP_PAIRS:
            ...
            s1 = sweep_fvg_scan(m15_candles=m15, h1_candles=None,
    ...
    if PROP_ENABLED:
        ...
        for pair in PROP_PAIRS:
            ...
            sigs = prop_scan(m15_candles=m15, h1_candles=None, pair=pair)
