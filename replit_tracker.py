...
def sheet_sweep_missed(sig: dict):
    """
    Mark SweepFVG row as MISSED.
    Price blew through TP without FVG retrace.
    Updates col 14 (status) only.
    """
...
def alert_sweep_missed(sig: dict, price: float):
    """
    Phase 0 — Setup missed alert.
    Fires when price blows through TP target
    without retracing into FVG first.
    Move has played out — no entry possible.
    """
    pair = sig['pair']
    dp   = DP.get(pair, 5)
...
def _poll():
    ...
    sweep_to_invalidate = []   # key only
    sweep_to_missed     = []   # (key, sig, price) — TP blown
    ...
    for key in pending_keys:
        ...
        if tp_blown:
            # Price reached TP target without retracing into FVG
            # Move has played out — setup missed entirely
            sweep_to_missed.append((key, sig, price))
            print(
                f"[PHASE0] {pair} {direction.upper()} "
                f"MISSED — TP blown through without entry "
                f"@ {price}"
            )
            continue
...
def _sync_sweep():
    ...
    Three cases per row:
      1. Key already in sweep_active  → skip (already monitoring)
      2. Key in sweep_pending but sheet now shows ACTIVE
         → promote: move from pending to active, send entry alert
         This handles the case where _poll() triggered entry and
         wrote ACTIVE to sheet, but the in-memory pending dict
         still held the old key — entry alert was never sent.
      3. Key not in either pool → add normally
    """
    sheet = get_sweep_sheet()
    if not sheet:
        return
    try:
        ...
        for row in sheet.get_all_values()[1:]:
            ...
            try:
                ...
                with trades_lock:
                    in_active  = key in sweep_active
                    in_pending = key in sweep_pending

                # Already tracked as active — nothing to do
                if in_active:
                    continue

                # fvg bounds in cols 19-20 (0-indexed 18-19)
                ...
                # ── Case: was pending, sheet now shows ACTIVE ─────────────
                # Entry triggered externally (e.g. by _poll on a previous
                # cycle that wrote ACTIVE to sheet but never promoted the
                # in-memory dict). Promote now and send the missed alert.
                if in_pending and status == 'ACTIVE':
                    entry_time = sig['entry_time'] or _now_str()
                    active_sig = {
                        **sig,
                        'entry_time': entry_time,
                        'tp1_hit'   : sig['tp1_hit'],
                    ...
                    active_sig = _restore_excursion(key, active_sig)
                    with trades_lock:
                        sweep_pending.pop(key, None)
                        sweep_active[key] = active_sig
                    promoted += 1
                    print(
                        f"[SYNC] PROMOTED pending→active: "
                        f"{pair} {direction.upper()}"
                    )
                    # Send the entry alert that was missed
                    alert_sweep_entry(sig, entry_time)
                    continue

                # ── Case: already pending, still pending — skip ───────────
                if in_pending and status == 'PENDING_ENTRY':
                    continue

                # ── Case: new row — add to appropriate pool ───────────────
                if status == 'PENDING_ENTRY':
                ...
                else:  # ACTIVE — new to us
                    active_sig = _restore_excursion(key, sig)
                    with trades_lock:
                        sweep_active[key] = active_sig
                    added_active += 1
                    print(
                    ...
                    # If entry_time is present and this is a fresh load
                    # (i.e. Render restarted mid-trade), send a recovery
                    # alert so the channel knows monitoring resumed
                    if sig['entry_time']:
                        _tg(
                            f"🔄 *{pair} — Monitoring Resumed*\n\n"
                            f"Direction : `{'SELL' if direction == 'short' else 'BUY'}`\n"
                            f"Entry     : `{float(sig['entry']):.{DP.get(pair,5)}f}`\n"
                            f"Entry time: `{sig['entry_time']}`\n"
                            f"TP1 hit   : `{sig['tp1_hit']}`\n\n"
                            f"_Render restarted — trade re-loaded from sheet._\n"
                            f"_⚡ EDGE Replit Tracker_"
                        )

            except Exception as e:
                print(f"[SYNC] Sweep row error: {e}")

        if added_pending or added_active or promoted:
            print(
