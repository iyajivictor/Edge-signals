...
==============================
Real-time TP1/TP2 monitoring for Sweep+FVG trades.
Kept alive on Replit free tier via UptimeRobot pinging /health every 5min.

Setup:
  1. Create Python Repl, paste this as main.py
  ...
  3. Add Secrets (lock icon):
       TG_TOKEN, TG_CHAT_ID, TWELVE_DATA_KEY, SHEET_ID, GOOGLE_CREDENTIALS
  4. Click Run — copy the URL shown
  5. Add URL to UptimeRobot (free) as HTTP monitor, every 5 minutes

How it works:
  · Flask server at port 8080 — responds to /health pings (keeps Replit alive)
  · Background thread polls prices every 10 seconds
...
Signal lifecycle this tracker handles:
  Phase 1 (tp1_hit=False):
    · TP1 hit → Telegram alert, move SL to BE, update sheet col 16
    · SL hit  → LOSS alert, close trade, update sheet
  Phase 2 (tp1_hit=True):
    · TP2 hit → FULL WIN alert, update sheet
    · BE hit  → TP1_BE alert, update sheet
...
def fetch_price(pair):
    ...
    try:
        p = requests.get(
            ...
            timeout=8).json().get("price")
        if p:
            _price_cache[pair]=float(p); _price_ts[pair]=now; return float(p)
    except Exception as e: print(f"[PRICE] {pair}: {e}")
    return None
