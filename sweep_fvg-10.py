...
class SweepDetector:
    ...
    def _find_sweep(self, zone: dict, entry_i: int) -> tuple:
        """
        Search N=14 candles from zone entry for a valid sweep candle.
        Monitors zone integrity on every bar.

        Returns tuple (reason, payload):
          ('found',     sweep_dict) — sweep confirmed
          ('exhausted', exit_i)     — N ran out, zone still valid,
                                      re-entry search may resume
          ('breakout',  exit_i)     — close through zone boundary,
                                      zone is dead for this side
        ...
        for i in range(entry_i, window_end):
            ...
            # ── Zone integrity check ──────────────────────────────────────
            if zone['side'] == 'BSL' and c['close'] > zone['top']:
                logger.info(
                    f"[{self.pair}] Zone BSL {zone['top']:.5f} "
            ...
            if zone['side'] == 'SSL' and c['close'] < zone['bottom']:
                logger.info(
                    f"[{self.pair}] Zone SSL {zone['bottom']:.5f} "
                    f"BREAKOUT @ bar {i} ({c['time']}) — zone dead"
                )
                return ('breakout', i)
        ...
        # N=14 exhausted without sweep or breakout
        exit_i = window_end
        logger.info(
            f"[{self.pair}] N=14 exhausted — "
            f"{zone['side']} {zone['top']:.5f} | "
            f"zone still valid, checking for re-entry"
        )

        if exit_i >= self.n:
            return ('end', None)

        return ('exhausted', exit_i)

    def detect(self) -> list:
        """
        Run sweep detection across all valid zones.

        Per zone, searches only the last RECENT_CANDLES of M15 history
        and loops through all zone entries within that window:
          FOUND     → record sweep, keep searching for a more recent one
          EXHAUSTED → N ran out, advance past window, look for re-entry
          BREAKOUT  → zone structurally dead, stop searching this zone
          END       → no more candle data, stop

        Returns at most one sweep per zone — the most recent valid one.
        """
        if self._sweeps is not None:
            return self._sweeps

        sweeps       = []
        search_start = max(0, self.n - self.RECENT_CANDLES)

        for side in ('bsl', 'ssl'):
            for zone in self.zones[side]:

                start_i     = search_start
                zone_sweeps = []
                attempts    = 0
                ...
                while start_i < self.n and attempts < MAX_ATTEMPTS:
                    ...
                    if entry_i is None:
                        logger.info(
                            ...
                            f"{zone['top']:.5f} — no entry from bar {start_i}"
                        )
                        break
                    ...
                    if reason == 'found':
                        ...
                        # Advance past this sweep — look for a more recent one
                        start_i = payload['sweep_i'] + 1

                    elif reason == 'exhausted':
                        # Zone still valid — search for re-entry
                        start_i = payload  # payload = exit_i
                        logger.info(
                            f"[{self.pair}] {zone['side']} "
                            f"re-entry search from bar {start_i}"
                        )

                    elif reason == 'breakout':
                        # Zone closed through — dead
                        logger.info(
                            f"[{self.pair}] {zone['side']} "
                            f"{zone['top']:.5f} dead after breakout"
                        )
                        break
                ...
                if zone_sweeps:
                    latest = max(zone_sweeps, key=lambda s: s['sweep_i'])
                    sweeps.append(latest)
                    logger.info(
                        f"[{self.pair}] {zone['side']} — "
                        f"using sweep @ {latest['sweep_time']} "
                        f"({len(zone_sweeps)} sweep(s) found in window)"
                    )
