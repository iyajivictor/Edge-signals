...
H4_SWING_N      = 3
H4_TP_LOOKBACK  = 40
SIGNAL_TTL_HOURS  = 4    # dedup cache — prevents same zone re-firing within 4h
MAX_SWEEP_AGE_M15 = 12   # drop signal if M15 sweep candle is older than 12 bars (~3h)
PENDING_TTL_MINS  = 60   # pending entry expires after 60 mins — based on backtest avg entry wait
...
def check_pending_entries(m15_candles, pair):
    ...
    for pk, sig in pair_pending.items():
        ...
        try:
            created_str = sig.get('created_at', '').replace(' UTC', '+00:00')
            # Handle both HH:MM and HH:MM:SS formats
            fmt = '%Y-%m-%d %H:%M:%S+00:00' if len(created_str) > 19 else '%Y-%m-%d %H:%M+00:00'
            try: created = datetime.strptime(created_str, fmt).replace(tzinfo=timezone.utc)
            except: created = datetime.fromisoformat(created_str)
        ...
        # Pending TTL — invalidate if older than PENDING_TTL_MINS
        # Based on backtest: winners enter in ~10 mins, losers in ~37 mins
        # Anything beyond 60 mins is a landmine — stale context, don't trigger
        try:
            created = datetime.fromisoformat(
                sig.get('created_at', '').replace(' UTC', '+00:00'))
            age_min = (datetime.now(timezone.utc) - created).total_seconds() / 60
            if age_min > PENDING_TTL_MINS:
                pending[pk]['status'] = 'INVALIDATED'
                logger.info(f"[{pair}] Pending expired ({age_min:.0f}m old, "
                            f"limit {PENDING_TTL_MINS}m) — landmine defused")
                continue
        except Exception:
            pass
...
def cleanup_expired_pending():
    ...
    for pk, sig in pending.items():
        ...
        try:
            ...
            if (now - created).total_seconds() / 60 > PENDING_TTL_MINS:
                drop.append(pk)
        except Exception:
            drop.append(pk)
