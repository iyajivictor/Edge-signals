...
def run_sweep_tracker(sweep_sheet):
    ...
    PENDING_ENTRY  → outcome tracker PRIMARY: monitors for entry trigger,
                     updates status to ACTIVE, Replit picks it up on next sync
    ACTIVE tp1=F   → Replit PRIMARY, outcome tracker FALLBACK only
    ACTIVE tp1=T   → Replit PRIMARY, outcome tracker FALLBACK only
    """
    if not sweep_sheet: print("  [SWEEP] No sheet"); return

    live          = load_sweep_live()
    pending_entry = {k: v for k, v in live.items()
                     if v.get('status') == 'PENDING_ENTRY'}
    phase1        = {k: v for k, v in live.items()
                     if v.get('status') == 'ACTIVE' and not v.get('tp1_hit', False)}
    phase2        = {k: v for k, v in live.items()
                     if v.get('status') == 'ACTIVE' and v.get('tp1_hit', False)}

    if not pending_entry and not phase1 and not phase2:
        print("  [SWEEP] No active signals"); return
    ...
    for key, sig in pending_entry.items():
        ...
        dp = DP.get(pair, 5)
        entry_triggered = price <= entry if direction == 'short' else price >= entry
        print(f"  [{pair}] Pending price={price:.{dp}f} entry={entry:.{dp}f}")

        if entry_triggered:
            print(f"  [{pair}] Entry triggered — Replit takes over")
            live[key]['status']   = 'ACTIVE'
            live[key]['tp1_hit']  = False
            try:
                row = _find_row(sweep_sheet, sig)
                if row:
                    sweep_sheet.update_cell(row, 14, 'ACTIVE')
                    sweep_sheet.update_cell(row, 15, now_str())
                    print(f"  [{pair}] Sheet updated to ACTIVE row {row}")
            except Exception as e:
                print(f"  [{pair}] Sheet update err: {e}")

    # ── PHASE 1 FALLBACK — Replit is primary ─────────────────────────────
    for key, sig in phase1.items():
        pair  = sig.get('pair'); direction = sig.get('direction')
        sl    = float(sig.get('sl', 0)); tp1 = float(sig.get('tp1', 0))
        if pair not in TD_SYMBOLS: continue
    ...
    for key, sig in phase2.items():
        pair  = sig.get('pair'); direction = sig.get('direction')
        entry = float(sig.get('entry', 0)); tp2 = float(sig.get('tp2', 0))
        if pair not in TD_SYMBOLS: continue
